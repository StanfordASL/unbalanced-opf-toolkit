...
% Now, we will try to replicate the paper's results.
% In the continuation analysis, the authors used a range of factors
% kappa \in [-1,2]
...
% Now we need to choose a linearization point for solving the approximate power flow.
% One common choice is the power flow solution at the first time of the base case.
% Here, the base case should be understood in the context of OPF problems:
% the case in the absence of controllable loads. However,
% since we are not working with controllable loads yet, this is simply the
% first time step of the load case.
% Here, it should become clear why we reordered kappa_vec above so that 1
% is the first element: so that we use it as our linearization point.
